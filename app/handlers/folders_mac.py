...
try:
    ...
    from app.handlers.pdf_reader_mac import read_pdf
    from app.handlers.size_checker import check_size
except ImportError:
    from txt_reader import read_txt
    from pdf_reader_mac import read_pdf
...
def extract_file_text(file_path: str) -> str:
    ...
    try:
        ...
        if ext in {".pptx", ".ppt"}:
            try:
                from app.handlers.pptx_reader_mac import pptx_to_pdf
            except ImportError:
                from pptx_reader_mac import pptx_to_pdf
            return _pdf_text(pptx_to_pdf(file_path))
        if ext in {".doc", ".docx"}:
            try:
                from app.handlers.word_reader_mac import read_word
            except ImportError:
                from word_reader_mac import read_word
            return _pdf_text(read_word(file_path))
        if ext in {".png", ".jpg", ".jpeg"}:
            try:
                from app.handlers.image_reader_mac import process_image
            except ImportError:
                from image_reader_mac import process_image
