...
from storage.chromadb.factory import get_chroma_client  # фабрика клиента chromadb
...
class LinkAction(str, Enum):
    ...
    new_root  — карточка начинает новую тему
    child_of  — карточка развивает существующую карточку
    update_of — карточка перезаписывает факт в существующей карточке
    """
    NEW_ROOT  = "new_root"
    CHILD_OF  = "child_of"
    UPDATE_OF = "update_of"
...
class ZettelVectorDB:
    ...
    def __init__(
        ...
        # фабрика поддерживает docker http и локальный persistent режим
        ...
        # восстанавливаем кэш из chromadb после перезапуска бота
...
class GraphLinker:
    ...
    def __init__(
        ...
        self.model_name = model_name
        # llm нужна только для решений child_of / update_of / new_root
        # эмбеддинги полностью локальные
        base_llm = ChatOpenAI(
    ...
    def link_and_insert(
        ...
        for card in new_cards:
            ...
            # ветка 1: дочерняя карточка внутри одного сообщения
            ...
            # ветка 2: корневая карточка — ищем место в существующей базе
            else:
    ...
    def _handle_root_card(
        ...
        # нет кандидатов — сразу new_root без llm
        if not candidates:
