...
16. [Оценка качества — RAG Retrieval](#16-оценка-качества--rag-retrieval)
17. [Оценка качества — RAG Generation](#17-оценка-качества--rag-generation)
...
| GraphRAG | `anthropic/claude-haiku-4.5` | 0.3 | Генерация ответов по графу |
...
│   ├── synthetic_datasets/        # Сгенерированные данные
│   ├── metric_results/            # Результаты, графики, отчёты
...
```mermaid
flowchart LR
    A["ZettelCard от Atomizer"] --> B["Linker smart model\n(gpt-5.2)\n→ ground_truth.jsonl"]
...
| Macro-F1 | Средний F1 по трём классам действий |
| Hit@1 | Попадание в правильный целевой узел |
| Edge Precision / Recall / F1 | Доля корректных рёбер в итоговом графе |
...
|-------|--------|---------|-------------|---------|
| 🥇 1 | `google/gemini-2.5-flash` | 0.9229 | 0.946 | 0.51s |
| 🥈 2 | `anthropic/claude-haiku-4.5` | 0.9069 | 0.936 | 1.05s |
| 🥉 3 | `openai/gpt-4o-mini` | 0.8455 | 0.901 | 0.56s |

`gemini-2.5-flash` лидирует с наивысшей точностью классификации действий (94.6%) и минимальной задержкой.
...
## 16. Оценка качества — RAG Retrieval
...
### 16.3 Результаты (по top_k)

| top_k | Precision | Recall | F1 | HitRate | MRR | nDCG |
|-------|-----------|--------|-----|---------|-----|------|
| 3 | 0.094 | 0.088 | 0.073 | 0.221 | 0.189 | 0.124 |
| 5 | 0.072 | 0.101 | 0.068 | 0.252 | 0.189 | 0.116 |
| **7** | **0.069** | **0.119** | **0.073** | **0.298** | **0.189** | **0.121** |

`top_k=7` показывает лучший баланс: максимальный Recall (0.119) и HitRate (0.298) при сохранении F1 на уровне `top_k=3`.

![Метрики retrieval по top_k (bar chart)](./eval/metric_results/rag/retrieval/figures/retrieval_metrics_bar.png)

![Группированные метрики retrieval](./eval/metric_results/rag/retrieval/figures/retrieval_grouped_bar.png)

---

## 17. Оценка качества — RAG Generation

### 17.1 Задача

Generation-компонент получает извлечённый контекст и генерирует текстовый ответ на вопрос. Оценивается LLM-судьёй (`gpt-5.1`).
...
| Overall | Среднее по всем метрикам |

### 17.3 Результаты

| Модель | Faithfulness | Relevance | Completeness | Coherence | Overall |
|--------|-------------|-----------|--------------|-----------|---------|
| 🥇 `claude-haiku-4.5` | **0.972** | **0.896** | **0.290** | 0.987 | **0.7462** |
| 🥈 `gemini-2.5-flash` | **0.972** | 0.882 | 0.257 | 0.979 | 0.7294 |
| 🥉 `gpt-4o-mini` | 0.934 | 0.880 | 0.230 | **0.987** | 0.7274 |

**claude-haiku-4.5** лидирует по генерации ответов: лучший по faithfulness, relevance и completeness. Именно поэтому эта модель выбрана для GraphRAG в продакшне, несмотря на более высокую стоимость — при поиске качество ответа критичнее.

Низкий Completeness (0.23–0.29) у всех моделей объясняется тем, что retrieval-компонент не всегда находит все релевантные узлы — это ограничение retrieval, а не генерации.

![Метрики генерации по моделям](./eval/metric_results/rag/generator/figures/generation_metrics_bar.png)

![Радарная диаграмма метрик генерации](./eval/metric_results/rag/generator/figures/generation_radar.png)

![Рейтинг моделей генерации](./eval/metric_results/rag/generator/figures/generation_ranking.png)
